...
void ID(const int& PC, int& ST, const bool& forwarding, int& sdvg,
    ...
    bool exstID = false;
    while (!stID && !forwarding && !exstID) // besk cickl
    {
        bool D = false;
        int q = 0;
        while (q < REGS.size())
        ...
        if (D) { pipe[PC][ST] = "- "; sdvg++; if (line == "id" && pipe[PC][ST - 1] == "- ") { exstID = true; } }
...
void EX(const int& PC, int& ST, const bool& forwarding, int& sdvg,
    ...
    bool& stEX, string& line, int& waitEX, vector<pair<string, int>>& EXs, bool& debug) {
    ...
    string& stAcc, string& jumpPrediction, bool& jumped, vector<pair<int, int>> cfg, vector<pair<string, int>>& EXs, bool& debug)
{
        ...
        else if (jumpPrediction == "F") // предсказываем, что перехода не будет
        {
            if (jumped == false) // угадали
            {
                ...
                if (end == 3)
                {
                    ...
                    if (_ST + 1 == ST) {
                        EX(PC, ST, forwarding, sdvg, pipe, REGS, IFs, stEX, ex, waitEX, EXs, debug);
                        triesEX++;
                        stEX = false;
    ...
    while (triesEX < amountEX) {
        EX(PC, ST, forwarding, sdvg, pipe, REGS, IFs, stEX, EXstring, waitEX, EXs, debug);
        triesEX++;
        stEX = false;
    }
    string outR;
